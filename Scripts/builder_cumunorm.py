...
# This code generates the vacancy relationship for an individual sensor using the cumulative normal 
# distribution. The vacancy relationship maps the raw sensor value to the probability of vacancy. 
# ----------------------------------------------------------------------------------------------------------
import pandas as pd
import numpy as np
import math
...
def Sigmoid(x, p1, p2):
    ...
    y.index = x.index
    return y
    #return 0.5*(1+math.erf((x-p1)/(p2*math.sqrt(2))))
