...
class pi_client(object):
    ...
    def _utc_to_local(self, data, local_zone):
        ...
        if (not data.index.tz):
            data.index = data.index.tz_localize('UTC')  # accounts for localtime shift
...
# Custom code added by Lisa M. Slaughter

def get_snapshot_by_point(self,
                            point_names,
                            label=None,
                            dataserver="s09KoOKByvc0-uxyvoTV1UfQVVRJTC1QSS1Q", # defaults to UCD buildings dataserver
                            WebID_dic=None
        ...
        """
        # if point_names is a list, downloads all of them, then merges them into a dataframe
